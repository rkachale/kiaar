        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-brown bold uppercase">Insert HeaderMenu <?= "- ". $institutes_details['INST_NAME'] ?>  (<?= $institutes_details['INST_SHORTNAME']?>)</span>
                </div>
            </div>          
            ...
            <div class="portlet-body form">
                <?php mk_hpostform($base_url.$page."_manipulate".(isset($data['menu_id'])?"/".$data['menu_id']:"")); ?>
                    <div class="form-body">
                        ...
                        // mk_hselect("data[institute_id]",_l('Select Page',$this),$institutes,"INST_ID","INST_NAME",isset($data['institute_id'])?$data['institute_id']:null,""._l("Select Institute",$this)."");
                        ...
                        <!-- <div class="form-group ">
                            <label for="data[public]" class="control-label col-lg-2">New Tab</label>
                            <div class="col-lg-10 col-sm-10">
                                True<input onclick="if($(this).attr('checked')=='checked'){ $(this).next().removeAttr('checked'); }else{ $(this).next().attr('checked','checked'); }" value="1" type="radio"  style="width: 20px" class="radio form-control" id="data[target]" name="data[target]">
                                False<input onclick="if($(this).attr('checked')=='checked'){ $(this).next().removeAttr('checked'); }else{ $(this).next().attr('checked','checked'); }" value="0" type="radio" style="width: 20px" name="data[target]" class="radio form-control" checked="">
                            </div>
                        </div> -->
                        <?php
    ...
    <div class="col-md-6">
        ...
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption">
                    <i class="icon-social-dribbble font-brown"></i>
                    <span class="caption-subject font-brown bold uppercase">Header Menu List</span>
                </div>
            </div>
            <div class="portlet-body form">
                                ...
                                <?php if(isset($item['sub_menu_data']) && count($item['sub_menu_data'])!=0){ ?>
                                    <?php $j=0; foreach($item['sub_menu_data'] as $item2){  ?>
                                        <tr>
                                            ...
                                            <td><i class="fa <?=(isset($item2["public"]) && $item2["public"]==1)?"fa-check":"fa-minus-circle"?>"></i>
                                            </td>
                                            <td>
                                                <a href="<?=$base_url?>edit<?=$page?>/institute/<?=$_SESSION['inst_id']?>/<?=$item2["menu_id"]?>" class="btn yellow-lemon mt-clipboard" title="<?=_l('Edit',$this)?>">
                                                <i title="<?=_l('Edit',$this)?>" class="fa fa-pencil"></i>Edit
                                                </a>
                                                <a href="<?=$base_url?>delete<?=$page?>/institute/<?=$_SESSION['inst_id']?>/<?=$item2["menu_id"]?>" class="btn red mt-clipboard" title="<?=_l('Delete',$this)?>" onClick="return doconfirm();">
                                                <i title="<?=_l('Delete',$this)?>" class="icon-delete"></i>Delete
                                                </a>
                                            </td>
                                        </tr>
                                        <?php if(isset($item['sub_menu_data'][$j]['sub_sub_menu_data']) && count($item['sub_menu_data'][$j]['sub_sub_menu_data'])!=0){  
                                            ...
                                            <?php $k=1; foreach($item['sub_menu_data'][$j]['sub_sub_menu_data'] as $item3){ ?>
                                                <tr class="gradeX" style="font-style: italic;">
                                                    ...
                                                    <td class="childme"><?=$item3["menu_order"]?></td>
                                                    <td><i class="fa <?=(isset($item3["public"]) && $item3["public"]==1)?"fa-check":"fa-minus-circle"?>"></i>
                                                    </td>
                                                    <td style="width: 100px;">
                                                        <a href="<?=$base_url?>edit<?=$page?>/institute/<?=$_SESSION['inst_id']?>/<?=$item3["menu_id"]?>" class="btn yellow-lemon mt-clipboard" title="<?=_l('Edit',$this)?>">
                                                        <i title="<?=_l('Edit',$this)?>" class="fa fa-pencil"></i>Edit
                                                        </a>
                                                        <a href="<?=$base_url?>delete<?=$page?>/institute/<?=$_SESSION['inst_id']?>/<?=$item3["menu_id"]?>" class="btn red mt-clipboard" title="<?=_l('Delete',$this)?>" onClick="return doconfirm();">
                                                        <i title="<?=_l('Delete',$this)?>" class="icon-delete"></i>Delete
                                                        </a>
                                                    </td>
                                                </tr>
